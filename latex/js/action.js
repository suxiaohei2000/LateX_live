export var action = {
    ...
    downloadJPG: function (callback) {
        ...
        image.onload = function () {
            ...
            let imageData = canvas_temp.toDataURL("image/jpeg");
            if (callback) {
                callback(imageData);
            } else {
                hiddenLink.href = imageData;
    ...
    init_message: function () {
        window.addEventListener('message', function (event) {
            ...
            /**
             * 参数 获取公式图片 {source: "zsw_latex", data: {method: "get", type: "png"}}
             * 参数 传递公式 {source: "zsw_latex", data: {method: "post", latex: "公式代码"}}
             */
            if (params.source == "zsw_latex") {
                const { method, latex, type = "png" } = params.data || {};
                if (!['get', 'post'].includes(method)) {
                    console.warn('method参数错误, 正确参数格式为{type: "zsw_latex", data: {method: "get|post", type: "png|jpg|svg", latex: "公式代码(post)"}}');
                }
                if (method == "get") {
                    let latex = action.obj_element.el_input.value;
                    let callback = (data) => {
                        action.postMessage({value: data, latex});
                    }
                    action.action(() => {
                        if (type == "png") {
                            action.downloadPNG(data => {
                                callback(data);
                            });
                        } else if (type == "jpg") {
                            action.downloadJPG(data => {
                                callback(data);
                            });
                        } else if (type == "svg") {
                            action.downloadSVG(data => {
                                callback(data);
                            });
                        }
                    }, "", "");
                    ...
                    if (latex) {
                        action.obj_input.input(latex, latex.length);
                    }
                }
            }
        })
    ...
    postMessage: function (data) {
        window.parent.postMessage({ source: "zsw_latex", data: {data} }, "*");
    },
}